...
namespace Business.ValidationRules.FluentValidation
{
    public class CarImagesOperationDtoValidator : AbstractValidator<CarImagesOperationDto>
    {
        public CarImagesOperationDtoValidator()
        {
            ...
            RuleFor(p => p.Images).Must(CheckIfImageLengh).WithMessage(Messages.ImageNotFound);
            RuleFor(p => p.Images).Must(CheckIfFileExtension).WithMessage(Messages.IncorrectFileExtension);
        }
